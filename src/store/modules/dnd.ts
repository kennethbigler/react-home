...
interface UpdatePlayerChoiceAction extends Action<typeof PLAYER_CHOICE> {
  playerChoice: Briefcase;
}
/** update the player's case choice in Deal or No Deal DB */
export const updatePlayerChoice = (
  playerChoice: Briefcase
): UpdatePlayerChoiceAction => ({ type: PLAYER_CHOICE, playerChoice });
...
interface SetOpenOfferAction extends Action<typeof OPEN_OFFER> {
  offer: number;
  casesToOpen: number;
}
/** display the offer from the banker and open modal in Deal or No Deal DB */
export const setOpenOffer = (
  offer: number,
  casesToOpen: number
): SetOpenOfferAction => ({ type: OPEN_OFFER, offer, casesToOpen });

interface SetNoDealAction extends Action<typeof NO_DEAL> {
  turn: number;
}
/** reject the offer and continue playing in Deal or No Deal DB */
export const setNoDeal = (turn: number): SetNoDealAction => ({
  type: NO_DEAL,
  turn,
});
...
export const finishGame = (offer: number): FinishGameAction => ({
  type: FINISH_GAME,
  offer,
});
...
type DNDActions =
  | NewGameAction
  | UpdatePlayerChoiceAction
  | SetOpenCaseAction
  | SetOpenOfferAction
  | SetNoDealAction
  | FinishGameAction;
export default function reducer(
  state: DBDND = initialState.dnd,
  action: DNDActions
): DBDND {
  switch (action.type) {
    case NEW_GAME:
