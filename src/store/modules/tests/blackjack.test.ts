...
import blackjackReducer, {
  ...
  stayHand,
  doubleHand,
  UPDATE_GAME_FUNCTIONS,
  UPDATE_HIDE_HANDS,
  UPDATE_HAS_FUNCTIONS,
} from "../blackjack";
import { TA } from "../turn";
...
const players = [newPlayer(1, "Ken", false), newPlayer(2)];
  ...
  describe("async thunk actions", () => {
    ...
    test("splitHand", () => {
      ...
      return store // @ts-expect-error: no idea why dispatch has this issue
        .dispatch(splitHand([{ cards: [queen, jack] }], 0, 0, defaultWeigh))
        .then(() => {
          // return of async actions
          expect(store.getActions()).toEqual(expectedActions);
        });
    ...
    test("hitHand", () => {
      ...
      const store = mockStore({});
      return store // @ts-expect-error: no idea why dispatch has this issue
        .dispatch(hitHand([{ cards: [queen] }], 0, 0, defaultWeigh))
        .then(() => {
          // return of async actions
          expect(store.getActions()).toEqual(expectedActions);
        });
      ...
      return store
        .dispatch(
          ...
          doubleHand(
            {
              ...players[0],
              hands: [{ cards: [queen], weight: 0, soft: false }],
            },
            { player: 0, hand: 0 },
            defaultWeigh
          )
