...
import playerReducer, {
  ...
  updateBet,
  payout,
  createSplitHandAction,
  createDrawCardAction,
  createSwapCardsAction,
  createNewHandAction,
  resetStatus,
  newHand,
  drawCard,
...
describe("store | modules | players", () => {
  describe("reducer", () => {
    test("actions", () => {
      ...
      expect(playerReducer(state, updateName(1, "Andrew"))).toEqual([
        state[0],
        { ...state[1], name: "Andrew" },
      ]);

      expect(playerReducer(state, updateBot(1, false))).toEqual([
        state[0],
        { ...state[1], isBot: false },
      ]);
      expect(playerReducer(state, updateBot(1))).toEqual([
        state[0],
        { ...state[1], isBot: true },
      ]);

      expect(playerReducer(state, updateBet(1, 69))).toEqual([
        state[0],
        { ...state[1], bet: 69 },
      ]);
      expect(playerReducer(state, updateBet())).toEqual([
        { ...state[0], bet: 5 },
        state[1],
      ]);

      expect(playerReducer(state, payout(1, "status", -31))).toEqual([
        state[0],
        { ...state[1], status: "status", money: 69 },
      ]);
      expect(
        playerReducer(state, createSplitHandAction(1, [{ cards: [king] }]))
      ).toEqual([state[0], { ...state[1], hands: [{ cards: [king] }] }]);
      expect(
        playerReducer(state, createDrawCardAction(1, [{ cards: [king] }]))
      ).toEqual([state[0], { ...state[1], hands: [{ cards: [king] }] }]);
      expect(
        playerReducer(state, createSwapCardsAction(1, [{ cards: [king] }]))
      ).toEqual([state[0], { ...state[1], hands: [{ cards: [king] }] }]);
      expect(
        playerReducer(state, createNewHandAction(1, [king], true, 10))
      ).toEqual([
        state[0],
  ...
  describe("async thunk actions", () => {
    test("newHand", () => {
      ...
      const expectedActions = [
        {
          type: PA.NEW_HAND,
          player: { id: 0, hands: [{ cards, weight: 0, soft: false }] },
        },
      ];
      ...
      const expectedActions = [
        {
          ...
          player: {
            id: 0,
            hands: [{ cards: [queen, king], weight: 0, soft: false }],
          },
        },
      ];
      const store = mockStore({ players: state });
      ...
      const expectedActions = [
        {
          ...
          player: {
            id: 0,
            hands: [{ cards: [queen, king] }, { cards: [jack, king] }],
          },
        },
      ];
      const store = mockStore({ todos: [] });
      return store // @ts-expect-error: no idea why dispatch has this issue
        .dispatch(splitHand([{ cards: [queen, jack] }], 0))
        .then(() => {
          // return of async actions
          expect(store.getActions()).toEqual(expectedActions);
        });
    ...
    test("swapCards", () => {
      ...
      const expectedActions = [
        {
          type: PA.SWAP_CARD,
          player: { id: 0, hands: [{ cards: [queen, king] }] },
        },
      ];
      const store = mockStore({ players: state });
      return store // @ts-expect-error: no idea why dispatch has this issue
        .dispatch(swapCards([{ cards: [queen, jack] }], 0, [1]))
        .then(() => {
          // return of async actions
          expect(store.getActions()).toEqual(expectedActions);
        });
