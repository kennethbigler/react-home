...
describe('games | poker | Poker', () => {
  it('renders as expected & plays a game', async () => {
    deck.mockReset()
      .mockResolvedValueOnce([card('A', '♠'), card('K', '♠'), card('Q', '♠'), card('J', '♠'), card('9', '♠')])
      // draw 4
      .mockResolvedValueOnce([card('K', '♣'), card('8', '♠'), card('7', '♠'), card('6', '♠'), card('5', '♠')])
      // draw 3
      .mockResolvedValueOnce([card('4', '♠'), card('3', '♠'), card('2', '♠'), card('2', '♣'), card('5', '♣')])
      // draw 1
      .mockResolvedValueOnce([card('3', '♣'), card('3', '♦'), card('3', '♥'), card('4', '♣'), card('6', '♣')])
      // draw 0
      .mockResolvedValueOnce([card('7', '♣'), card('8', '♣'), card('9', '♣'), card('10', '♣'), card('J', '♣')])
      // user redraws 1
