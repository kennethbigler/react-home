...
import {
  weighHand, playBot, banking, DEALER,
...
const BlackJack: React.FC = () => {
  ...
  const dealHands = React.useCallback((players: DBPlayer[]): void => {
    // shuffle the deck
    Deck.shuffle().then(() => {
      // deal the hands
  ...
  /** Start a new game */
  const newGame = React.useCallback((players: DBPlayer[]): void => {
    dispatch(setNewGame(players));
  }, [dispatch]);

  /** function to finish betting and start the game */
  const finishBetting = React.useCallback((players: DBPlayer[]): void => {
    dispatch(updateHideHands(false));
    dealHands(players);
  }, [dispatch, dealHands]);

  /** extract dispatch call to payout player */
  const payoutPlayer = React.useCallback((id: number, status: string, money: number) => {
    dispatch(payout(id, status, money));
  }, [dispatch]);

  /** function to get a new card */
  const hitDealer = React.useCallback((players: DBPlayer[]): void => {
    // get state values
  ...
  const playDealer = React.useCallback((players: DBPlayer[]): void => {
  ...
  const handleGameFunctionClick = (type: string): void => {
    switch (type) {
      case GameFunctions.NEW_GAME:
        newGame(players); break;
      case GameFunctions.FINISH_BETTING:
        finishBetting(players); break;
