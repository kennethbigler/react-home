...
describe('resume | graphql | mutations', () => {
  test('updateWatch', () => {
    const readFragment = jest.fn().mockReturnValueOnce(undefined).mockReturnValueOnce({ watchers: { totalCount: 68 }});
    ...
    // @ts-expect-error: mocking for tests
    expect(updateWatch(cache, { data: { updateSubscription: { subscribable: { id: 42, viewerSubscription: 69 }}}})).toBeUndefined();
  ...
  test('updateAddStar', () => {
    const readFragment = jest.fn().mockReturnValueOnce(undefined).mockReturnValueOnce({ stargazers: { totalCount: 68 }});
    const writeFragment = jest.fn();
    const cache = {
      readFragment,
      writeFragment,
    ...
    expect(writeFragment).toHaveBeenCalledTimes(0);
    // @ts-expect-error: mocking for tests
    expect(updateAddStar(cache, { data: { addStar: { starrable: { id: 42 }}}})).toBeUndefined();
    expect(readFragment).toHaveBeenCalledTimes(1);
    expect(writeFragment).toHaveBeenCalledTimes(0);
    // @ts-expect-error: mocking for tests
    expect(updateAddStar(cache, { data: { addStar: { starrable: { id: 42 }}}})).toBeUndefined();
  ...
  test('updateRemoveStar', () => {
    ...
    expect(writeFragment).toHaveBeenCalledTimes(0);
    // @ts-expect-error: mocking for tests
    expect(updateRemoveStar(cache, { data: { removeStar: { starrable: { id: 42 }}}})).toBeUndefined();
    expect(readFragment).toHaveBeenCalledTimes(1);
    expect(writeFragment).toHaveBeenCalledTimes(0);
    // @ts-expect-error: mocking for tests
    expect(updateRemoveStar(cache, { data: { removeStar: { starrable: { id: 42 }}}})).toBeUndefined();
