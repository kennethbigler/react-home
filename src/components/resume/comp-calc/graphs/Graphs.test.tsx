...
describe("Graphs", () => {
  ...
  const mockCompCalcEntries: CompCalcEntry[] = [
    ...
    { company: "Company C", date: "2022-01", stock: 70000, stockAdj: undefined },
  ];

  it("renders both CompChart and BreakdownChart", () => {
    render(<Graphs compEntries={mockCompEntries} compCalcEntries={mockCompCalcEntries} />);
  ...
  it("initializes with last entry values when stockAdj is available", () => {
    ...
    render(<Graphs compEntries={mockCompEntries.slice(0, 2)} compCalcEntries={entriesWithStockAdj} />);
  ...
  it("initializes with stock value when stockAdj is undefined", () => {
    const entriesWithoutStockAdj: CompCalcEntry[] = [
      { company: "Company A", date: "2020-01", stock: 50000, stockAdj: undefined },
      { company: "Company B", date: "2021-01", stock: 60000, stockAdj: undefined },
    ];

    render(<Graphs compEntries={mockCompEntries.slice(0, 2)} compCalcEntries={entriesWithoutStockAdj} />);
  ...
  it("uses stockAdj over stock when both are available during click", () => {
    ...
    render(<Graphs compEntries={mockCompEntries.slice(0, 1)} compCalcEntries={entriesWithBoth} />);
