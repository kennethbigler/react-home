...
describe('resume | Menu', () => {
  it('renders as expected', () => {
    ...
    expect(screen.getByText('Poker Night Scores')).toBeInTheDocument();
    expect(screen.getByText('React Games')).toBeInTheDocument();
    expect(screen.getByText('GitHub')).toBeInTheDocument();
    expect(screen.getByText('LinkedIn')).toBeInTheDocument();
    expect(screen.getByAltText('profile for Ken Bigler at Stack Overflow, Q&A for professional and enthusiast programmers')).toBeInTheDocument();
  });
  ...
  it('links to external sites', () => {
    ...
    expect(windowOpen).toHaveBeenCalledWith('https://github.com/kennethbigler/react-home');

    fireEvent.click(screen.getByText('LinkedIn'));
    expect(windowOpen).toHaveBeenCalledWith('https://www.linkedin.com/in/kennethbigler');

    fireEvent.click(screen.getByAltText('profile for Ken Bigler at Stack Overflow, Q&A for professional and enthusiast programmers'));
    expect(windowOpen).toHaveBeenCalledWith('https://stackoverflow.com/users/4830309/ken-bigler');
