...
import StreamExampleV2 from "./StreamExample";
...
describe("resume | a11y-practice | v2 StreamExample", () => {
  beforeEach(() => {
  ...
  it("flushes on debounce when tokens stall mid-sentence", async () => {
    mockedTokenStream.mockReturnValue(
      delayedStreamFromTokens(["mid sentence", " continues."], 0, 10_000),
    );
    ...
    await waitFor(() => {
      expect(screen.getByRole("status")).toHaveTextContent("mid sentence");
    });
  });
  ...
  it("stops updating when unmounted during an active stream", async () => {
    ...
    const { unmount } = render(<StreamExampleV2 />);
    clickStartStream();
